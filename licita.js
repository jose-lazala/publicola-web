...
// ============================================================
// SECCION "BUSCADOR DE RUBRO" -- vista de CONTEO por rubro.
//
// REGLA DE ORO (obligatoria): esta vista muestra SOLO el numero de
...
// y mapaProcesosPorFamilia, ya cargados por cargarDatosLicitaciones), asi
// que se inicializa despues de que esos datos terminen de cargar, nunca
// antes ni por separado.
// ============================================================

// Cuantas sugerencias de rubro como maximo se muestran mientras el
// visitante escribe.
const MAXIMO_SUGERENCIAS_RUBRO = 8;

let elementoCampoBuscadorRubro = null;
let elementoListaBuscadorRubro = null;
let elementoResultadoBuscadorRubro = null;
let listaRubrosPlana = [];

// Quita tildes y pasa a minusculas, para que la busqueda encuentre
// "energia" aunque el rubro real diga "Energía" (y al reves).
...
// Aplana datosAreas (areas -> rubros) en una sola lista de
// { codigoFamilia, rubro } para poder filtrarla directo por texto. Si una
// entrada viene incompleta o con forma inesperada, se descarta en vez de
// romper el resto de la lista.
function aplanarRubros(areas) {
  const plana = [];
  if (!Array.isArray(areas)) {
    return plana;
  }
  for (const entradaArea of areas) {
    if (!entradaArea || !Array.isArray(entradaArea.rubros)) {
      continue;
    }
    for (const entradaRubro of entradaArea.rubros) {
      if (entradaRubro && entradaRubro.codigo_familia && entradaRubro.rubro) {
        plana.push({ codigoFamilia: entradaRubro.codigo_familia, rubro: entradaRubro.rubro });
      }
    }
  }
  return plana;
...
// Cruza el rubro elegido contra mapaProcesosPorFamilia (por codigo_familia)
// y arma el texto de resultado usando UNICAMENTE total_abiertos -- nunca
// el arreglo "muestra". Si el cruce no encuentra datos para ese rubro (dato
// faltante o formato inesperado), se trata igual que "cero procesos", nunca
// se inventa un numero.
function construirResultadoRubro(rubro) {
  const datosRubro = mapaProcesosPorFamilia.get(rubro.codigoFamilia);
  const totalAbiertos =
    datosRubro && typeof datosRubro.total_abiertos === "number" && datosRubro.total_abiertos > 0
      ? datosRubro.total_abiertos
      : 0;
...
function mostrarResultadoRubro(rubro) {
  try {
    ...
    elementoResultadoBuscadorRubro.innerHTML = "";
    elementoResultadoBuscadorRubro.appendChild(crearElemento("p", null, resultado.texto));

    if (resultado.mostrarEnlacePrimeraEdicion) {
      const enlace = document.createElement("a");
    ...
    console.error("Publicola: fallo al cruzar los datos del rubro elegido", error);
    mostrarErrorBuscadorRubro();
  }
}

// Filtra listaRubrosPlana por el texto escrito (insensible a mayusculas y
// tildes, coincidencia parcial) y pinta la lista de sugerencias.
function renderizarSugerenciasRubro(textoEscrito) {
  elementoListaBuscadorRubro.innerHTML = "";

  const textoNormalizado = normalizarTextoBusqueda(textoEscrito.trim());
  if (textoNormalizado === "") {
    elementoListaBuscadorRubro.hidden = true;
    return;
  }

  const coincidencias = listaRubrosPlana
    .filter(function (rubro) {
      return normalizarTextoBusqueda(rubro.rubro).includes(textoNormalizado);
    })
    .slice(0, MAXIMO_SUGERENCIAS_RUBRO);
  ...
  for (const rubro of coincidencias) {
    const item = document.createElement("li");
    const boton = document.createElement("button");
    boton.type = "button";
    boton.className = "buscador-rubro-opcion";
    boton.textContent = rubro.rubro;
    boton.addEventListener("click", function () {
      elementoCampoBuscadorRubro.value = rubro.rubro;
      elementoListaBuscadorRubro.innerHTML = "";
      elementoListaBuscadorRubro.hidden = true;
      mostrarResultadoRubro(rubro);
    });
    item.appendChild(boton);
    elementoListaBuscadorRubro.appendChild(item);
  }

  elementoListaBuscadorRubro.hidden = false;
}

// Estado de error: se usa si rubros.json / procesos_semana.json fallaron al
// cargar (mismo error que ya maneja mostrarErrorLicitacionesSemana), para
// que el buscador de rubro nunca quede a medio armar ni sin explicacion.
function mostrarErrorBuscadorRubro() {
  if (elementoCampoBuscadorRubro) {
    elementoCampoBuscadorRubro.disabled = true;
    elementoCampoBuscadorRubro.placeholder = "En este momento no podemos cargar los rubros";
  }
  if (elementoListaBuscadorRubro) {
    elementoListaBuscadorRubro.innerHTML = "";
    elementoListaBuscadorRubro.hidden = true;
  }
  if (elementoResultadoBuscadorRubro) {
    elementoResultadoBuscadorRubro.innerHTML = "";
    elementoResultadoBuscadorRubro.appendChild(crearElemento("p", "mensaje-estado", MENSAJE_ERROR_CARGA));
    elementoResultadoBuscadorRubro.hidden = false;
  }
}

// Arma el buscador de rubro: campo de texto + lista de sugerencias en vivo
// + panel de resultado. Se llama una sola vez, despues de que datosAreas y
// mapaProcesosPorFamilia ya esten listos.
function inicializarBuscadorRubro() {
  const contenedor = document.getElementById("conteo-rubro");
  if (!contenedor) {
    return;
  }

  elementoCampoBuscadorRubro = document.getElementById("buscador-rubro-campo");
  elementoListaBuscadorRubro = document.getElementById("buscador-rubro-lista");
  elementoResultadoBuscadorRubro = document.getElementById("buscador-rubro-resultado");

  if (!elementoCampoBuscadorRubro || !elementoListaBuscadorRubro || !elementoResultadoBuscadorRubro) {
    return;
  }

  listaRubrosPlana = aplanarRubros(datosAreas);

  elementoCampoBuscadorRubro.addEventListener("input", function () {
    elementoResultadoBuscadorRubro.hidden = true;
    elementoResultadoBuscadorRubro.innerHTML = "";
    renderizarSugerenciasRubro(elementoCampoBuscadorRubro.value);
  });

  // Un clic o toque fuera de la lista de sugerencias (y fuera del propio
  // campo) la cierra, mismo patron que el menu de navegacion y el panel
  // de Licita mas abajo en este archivo.
  document.addEventListener("click", function (evento) {
    if (elementoListaBuscadorRubro.hidden) {
      return;
    }
    if (elementoListaBuscadorRubro.contains(evento.target) || elementoCampoBuscadorRubro.contains(evento.target)) {
      return;
    }
    elementoListaBuscadorRubro.hidden = true;
  });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  cargarDatosLicitaciones()
    .then(function (datos) {
      ...
      // El buscador de rubro depende de los mismos datosAreas y
      // mapaProcesosPorFamilia: se arranca aqui, ya con los datos listos,
      // nunca antes.
      arrancarConProteccion("inicializarBuscadorRubro", function () { inicializarBuscadorRubro(); });
    })
